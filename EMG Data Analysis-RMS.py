...
# -*- coding: utf-8 -*-
"""
Created on Wed May  6 11:58:46 2026
...
# ---- Calculate RMS ----
RMS_value = np.sqrt(np.mean(env**2))

# ---- Threshold = 75% of RMS ----
ON_thr = 0.75 * RMS_value

print(f"RMS value = {RMS_value:.2f} µV")
print(f"75% RMS Threshold = {ON_thr:.2f} µV")
...
plt.axhline(ON_thr,
            ...
            linestyle='--',
            linewidth=2,
            label=f"75% RMS Threshold ({ON_thr:.2f} µV)")

plt.title("Envelope with RMS and 75% RMS Threshold")
plt.xlabel("Time (s)")
